...
function IntensityBar({ score }: { score: number }) {
  ...
  const color = score >= 8 ? "bg-red-500" : score >= 6 ? "bg-amber-500" : score >= 4 ? "bg-yellow-400" : "bg-green-400";
  return (
...
function VendorFeatureSelector({ value, onChange }: {
  ...
  function handleFeatureChange(featureId: string) {
    ...
    for (const cat of library.categories) {
      ...
      if (feat) {
        onChange({ vendorId: library.vendorId, vendorName: library.vendorName,
          featureId: feat.featureId, featureName: feat.name,
          categoryName: cat.categoryName, rationale: value?.rationale ?? "" });
        return;
      }
    }
  }
  ...
  return (
    <div className="mt-1.5 space-y-1.5">
      <select value={vendorId} onChange={(e) => setVendorId(e.target.value)}
        ...
        {VENDOR_LIBRARIES.map((l) => <option key={l.vendorId} value={l.vendorId}>{l.vendorName}</option>)}
      </select>
      <select value={value?.featureId ?? ""} onChange={(e) => handleFeatureChange(e.target.value)}
        className="w-full rounded border border-gray-200 bg-white px-1.5 py-1 text-[10px] text-gray-700 focus:outline-none focus:ring-1 focus:ring-emerald-400">
      ...
      {value?.featureId && (
        <textarea placeholder="Why does this feature address this friction? (1-2 sentences)"
...
function SolutionCard({ solution, editMode, onUpdate }: {
  solution: Solution; editMode: boolean; onUpdate: (patch: Partial<Solution>) => void;
}) {
  ...
  return (
    <div className={`rounded border ${c.border} ${c.bg} p-2`}>
      <div className="mb-1.5">
        {editMode ? (
          <select value={solution.type} onChange={(e) => onUpdate({ type: e.target.value as SolutionType })}
          ...
          <span className={`rounded px-1.5 py-0.5 text-[10px] font-semibold ${c.badge}`}>{solution.type}</span>
        )}
      </div>
      {editMode ? (
        <textarea value={solution.description} onChange={(e) => onUpdate({ description: e.target.value })}
          rows={2} className="w-full resize-none rounded border border-gray-200 p-1 text-[10px] leading-relaxed text-gray-700 focus:outline-none focus:ring-1 focus:ring-violet-400" />
      ) : (
        <p className="text-[10px] leading-relaxed text-gray-700">{solution.description}</p>
      )}
      {solution.type === "Technology" && editMode && (
        <VendorFeatureSelector value={solution.vendorFeatureRef} onChange={(ref) => onUpdate({ vendorFeatureRef: ref })} />
      )}
      {solution.type === "Technology" && !editMode && solution.vendorFeatureRef && (
        <div className="mt-1.5 rounded border border-[#b3d9f5] bg-[#e8f4fd] px-2 py-1.5">
          ...
          <p className="mt-0.5 text-[10px] font-medium text-[#0070d2]">{solution.vendorFeatureRef.featureName}</p>
          {solution.vendorFeatureRef.rationale && (
            <p className="mt-0.5 text-[10px] leading-relaxed text-gray-600">{solution.vendorFeatureRef.rationale}</p>
          )}
        </div>
      )}
    </div>
...
function NewSolutionForm({ onSave, onCancel }: {
  onSave: (s: Solution) => void; onCancel: () => void;
}) {
  ...
  return (
    <div className="rounded border border-dashed border-violet-300 bg-violet-50/30 p-2 space-y-1.5">
      ...
      <select value={type} onChange={(e) => setType(e.target.value as SolutionType)}
      ...
      <textarea placeholder="Describe the solution action..." value={description}
      ...
      <div className="flex justify-end gap-2 pt-0.5">
        <button onClick={onCancel} className="rounded px-2 py-1 text-[10px] text-gray-500 hover:bg-gray-100">Cancel</button>
        <button onClick={() => { if (!description.trim()) return;
          onSave({ solutionId: `sol-${Date.now()}`, type, description: description.trim(),
            vendorFeatureRef: type === "Technology" ? vendorFeatureRef : undefined }); }}
          disabled={!description.trim()}
          className="rounded bg-violet-600 px-2 py-1 text-[10px] font-semibold text-white hover:bg-violet-700 disabled:opacity-40">Add</button>
      </div>
    </div>
  );
}

function SolutionsSection({ solutions, editMode, onUpdate, onAdd }: {
  solutions: Solution[]; editMode: boolean;
  ...
  return (
    <div className="mt-2 border-t border-gray-100 pt-2">
      ...
      {expanded && (
        <div className="mt-1.5 space-y-1.5">
          {solutions.map((sol) => (
            <SolutionCard key={sol.solutionId} solution={sol} editMode={editMode}
              onUpdate={(patch) => onUpdate(sol.solutionId, patch)} />
          ))}
          {editMode && (showNewForm ? (
            <NewSolutionForm onSave={(s) => { onAdd(s); setShowNewForm(false); }} onCancel={() => setShowNewForm(false)} />
          ) : (
...
function ObservationCard({ obs, isBindingObs, scaffold, editMode, onUpdate }: {
  obs: FrictionObservation; isBindingObs: boolean; scaffold: ScaffoldData;
  editMode: boolean; onUpdate: (patch: Partial<FrictionObservation>) => void;
}) {
  ...
  function anchorName(anchorType: string, anchorId: string): string {
    const map = anchorType === "Activity" ? scaffold.elements.activities :
    ...
    const el = map?.[anchorId] as { name?: string } | undefined;
    return el?.name ?? anchorId;
  }
  return (
    <div className={`rounded-md border border-gray-100 border-l-4 ${borderColor} bg-white p-3 ${isBindingObs ? "ring-1 ring-red-200 ring-offset-1" : ""} ${editMode ? "ring-1 ring-violet-200" : ""}`}>
      <div className="flex items-start justify-between gap-2">
        <div className="flex items-center gap-2">
          {editMode ? (
            <select value={obs.category} onChange={(e) => onUpdate({ category: e.target.value })}
              ...
              {CATEGORY_OPTIONS.map((cat) => <option key={cat} value={cat}>{categoryLabel(cat)}</option>)}
            </select>
          ) : (
            <span className={`rounded px-1.5 py-0.5 text-[10px] font-semibold ${group === "execution" ? "bg-amber-50 text-amber-700" : "bg-red-50 text-red-700"}`}>
              {categoryLabel(obs.category)}
            </span>
          )}
          {isBindingObs && <span className="rounded bg-red-500 px-1.5 py-0.5 text-[10px] font-semibold text-white">Binding</span>}
        </div>
        {editMode ? (
          <div className="flex items-center gap-1">
            <input type="number" min={0} max={10} step={0.5} value={obs.intensity.score ?? 0}
        ...
        ) : obs.intensity.score != null ? <IntensityBar score={obs.intensity.score} /> : null}
      </div>
      {editMode ? (
        <textarea value={obs.rationale} onChange={(e) => onUpdate({ rationale: e.target.value })}
          rows={3} className="mt-2 w-full resize-none rounded border border-gray-200 p-1.5 text-xs leading-relaxed text-gray-700 focus:outline-none focus:ring-1 focus:ring-violet-400" />
      ) : (
      ...
      <div className="mt-2 flex flex-wrap items-center gap-x-4 gap-y-1 text-[10px] text-gray-500">
        <span>Anchor: {obs.primaryAnchor.anchorType} / {anchorName(obs.primaryAnchor.anchorType, obs.primaryAnchor.anchorId)}</span>
        {obs.confidence != null && <span>Confidence: {(obs.confidence * 100).toFixed(0)}%</span>}
      </div>
      {obs.contributingAnchors && obs.contributingAnchors.length > 0 && (
        <div className="mt-1.5 flex flex-wrap gap-1">
          {obs.contributingAnchors.map((ca) => (
            <span key={`${ca.anchorType}-${ca.anchorId}`}
      ...
      <SolutionsSection
        ...
        onUpdate={(solId, patch) => {
          const solutions = (obs.solutions ?? []).map((s) => s.solutionId === solId ? { ...s, ...patch } : s);
          onUpdate({ solutions });
        }}
...
function NewObservationForm({ activityId, onSave, onCancel }: {
  activityId: string; onSave: (obs: FrictionObservation) => void; onCancel: () => void;
}) {
  ...
  return (
    <div className="rounded-md border border-violet-200 bg-violet-50/40 p-3">
      <p className="mb-2 text-[10px] font-semibold uppercase tracking-wider text-violet-600">New Observation</p>
      ...
      <textarea placeholder="Describe the friction observation..." value={rationale}
        onChange={(e) => setRationale(e.target.value)} rows={3}
      ...
      <div className="mt-2 flex justify-end gap-2">
        <button onClick={onCancel} className="rounded px-2 py-1 text-[10px] text-gray-500 hover:bg-gray-100">Cancel</button>
        <button onClick={() => { if (!rationale.trim()) return;
          onSave({ observationId: `obs-${Date.now()}`, category,
            primaryAnchor: { anchorType: "Activity", anchorId: activityId },
            intensity: { scale: "0-10", score }, rationale: rationale.trim(),
            observedAt: new Date().toISOString(), solutions: [] }); }}
...
export function FrictionPanel({ activityId, observations: initialObservations, heatmap, scaffold, onClose }: {
  activityId: string; observations: FrictionObservation[];
  heatmap: HeatmapData; scaffold: ScaffoldData; onClose: () => void;
}) {
  ...
  function handleUpdate(observationId: string, patch: Partial<FrictionObservation>) {
    setObservations((prev) => prev.map((o) => o.observationId === observationId ? { ...o, ...patch } : o));
  }
  function handleExport() {
    const exported = { ...heatmap, observations: [
      ...heatmap.observations.filter((o) => !observations.find((eo) => eo.observationId === o.observationId)),
      ...observations] };
    const blob = new Blob([JSON.stringify(exported, null, 2)], { type: "application/json" });
    const url = URL.createObjectURL(blob);
    const a = document.createElement("a"); a.href = url;
    a.download = `${heatmap.heatmapId}-edited-${Date.now()}.json`; a.click();
    ...
    <div className="flex h-full flex-col border-l border-gray-100 bg-gray-50/30">
      <div className="flex items-center justify-between border-b border-gray-200 px-4 py-3">
        <div>
          ...
          <p className="mt-0.5 flex gap-2 text-[10px] text-gray-500">
            {execCount > 0 && <span className="rounded bg-amber-50 px-1 text-amber-700">{execCount} execution</span>}
            {govCount > 0 && <span className="rounded bg-red-50 px-1 text-red-700">{govCount} governing</span>}
          </p>
        </div>
        <div className="flex items-center gap-1">
          <button onClick={handleExport} title="Download edited heatmap"
          ...
          {editMode ? (
            <>
              <button onClick={() => { setObservations(savedObservations); setEditMode(false); setShowNewForm(false); }}
                className="rounded px-2 py-1 text-[10px] text-gray-500 hover:bg-gray-100">Cancel</button>
              <button onClick={() => { setSavedObservations(observations); setEditMode(false); setShowNewForm(false); }}
                className="rounded bg-violet-600 px-2 py-1 text-[10px] font-semibold text-white hover:bg-violet-700">Save</button>
            </>
          ) : (
            <button onClick={() => setEditMode(true)} title="Edit observations"
              className="rounded p-1 text-gray-400 transition-colors hover:bg-gray-100 hover:text-gray-600">
              <svg className="h-4 w-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
      ...
      {editMode && (
        <div className="border-b border-violet-100 bg-violet-50/60 px-4 py-1.5">
          <p className="text-[10px] font-medium text-violet-600">Editing — changes are in-memory only until exported</p>
        </div>
      )}
        ...
        heatmap.bindingConstraint.bindingAnchor.anchorId === activityId && (
          <>
            <div className="border-b border-red-100 bg-red-50/60 px-4 py-3">
              <p className="text-[10px] font-semibold uppercase tracking-wider text-red-600">Binding Constraint</p>
              <p className={`mt-1 text-xs leading-relaxed text-red-800 ${!justificationExpanded ? "line-clamp-3" : ""}`}>
                {heatmap.bindingConstraint.justification}
              </p>
              {heatmap.bindingConstraint.justification.length > 150 && (
                <button onClick={() => setJustificationExpanded(!justificationExpanded)}
              ...
              {heatmap.bindingConstraint.confidence != null && (
                <p className="mt-1 text-[10px] text-red-600">Confidence: {(heatmap.bindingConstraint.confidence * 100).toFixed(0)}%</p>
              )}
            </div>
      ...
      <div className="flex-1 space-y-2 overflow-auto p-4">
        {sorted.map((obs) => (
          <ObservationCard key={obs.observationId} obs={obs}
            isBindingObs={obs.observationId === bindingObsId}
            scaffold={scaffold} editMode={editMode}
            onUpdate={(patch) => handleUpdate(obs.observationId, patch)} />
        ))}
        {editMode && (showNewForm ? (
          <NewObservationForm activityId={activityId}
